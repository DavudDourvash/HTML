...
file.copy(file.path(ifelse(Sys.info()['sysname']=="Windows", "C:\\Users\\Chris\\Dropbox\\R-files\\HTML\\sorttable.js",
                           "~/Dropbox/R-files/HTML/sorttable.js"),  fsep = .Platform$file.sep),
          file.path(getwd(), "temp/sorttable.js",  fsep = .Platform$file.sep))
...
Quarter <<- 18 ### Mark current quarter here
...
mydatafirst$Date = as.Date(mydatafirst$Date, format = "%d/%m/%Y")

mydatafirst$Year = as.numeric(format(mydatafirst$Date, "%Y"))

mydatafirst$Month = as.numeric(format(mydatafirst$Date, "%m"))

### convert to Time code

mydatafirst$timeTemp = NA

mydatafirst$timeTemp[mydatafirst$Year == 2013 & mydatafirst$Month %in% 4:6] = 17
mydatafirst$timeTemp[mydatafirst$Year == 2013 & mydatafirst$Month %in% 1:3] = 16
mydatafirst$timeTemp[mydatafirst$Year == 2012 & mydatafirst$Month %in% 10:12] = 15
mydatafirst$timeTemp[mydatafirst$Year == 2012 & mydatafirst$Month %in% 7:9] = 14
mydatafirst$timeTemp[mydatafirst$Year == 2012 & mydatafirst$Month %in% 4:6] = 13
mydatafirst$timeTemp[mydatafirst$Year == 2012 & mydatafirst$Month %in% 1:3] = 12
mydatafirst$timeTemp[mydatafirst$Year == 2011 & mydatafirst$Month %in% 10:12] = 11

mydatafirst$Time[is.na(mydatafirst$Time)] = mydatafirst$timeTemp[is.na(mydatafirst$Time)]
...
mybar=function(PlaceN, PlaceC, GraphN, type){
    ...
    #    PlaceC = 722 ; PlaceN = "TeamC"; GraphN = 1; type = "main"
...
mytable=function(PlaceN, PlaceC, x){
    ...
    if(x==1){ 
        ...
        myCat(paste("<p>The table below shows the percentage of responses from the comments received to the question
           'If you could improve one thing about the care received what would it be?'  We received", addcomments1,
                    "responses to this question this quarter and", addcomments2, "responses in the last four quarters.
                    The table can be re-ordered by clicking any column heading. To reset the table to its original layout,
                    please use the following button.</p>")) 
    ...
    myCat('<input type="button" value="Reset table" onClick="window.location.reload()">')
    
    print(xtable(finaltable3), type="html", html.table.attributes = 'class = "sortable"', append=TRUE, file=currentPath, include.rownames=FALSE) 
...
mycomments=function(PlaceN, PlaceC, type, split){ 
    ...
    if(type==1){  
        ...
        commentstable = subset(mydata, Time==Quarter)[,c("TeamC", "Improve", "Imp1")]
        
        names(commentstable) = c("TeamC", "Improve", "Imp1")
    ...
    if(type==2){  
        ...
        commentstable=subset(mydata, Time==Quarter)[,c("TeamC", "Best", "Best1")]
    ...
    for(x in 1:length(levels(factor(exampletable$Super)))){ # make a lookup table for comments
    ...
    if(length(levels(factor(exampletable$Super)))>0){ 
        ...
        for(y in 1:length(levels(factor(exampletable$Super)))){ 
            ...
            if(sum(commentstable$Imp1 %in% unlist(store[[y]]))>0){ 
                ...
                if(split == "Directorate") myCat(c("<h3> ", levels(factor(exampletable$Super))[y], " </h3>"))
                
                vecprint=subset(commentstable, Imp1 %in% unlist(store[[y]]))[, c("TeamC", "Improve")]
                
                vecprint$TeamN = sapply(vecprint$TeamC, function(team)
                    as.character(subset(mydatafirst, Time==Quarter)$TeamN[tail(which(team == subset(mydatafirst,
                                                                                                    Time == Quarter)$TeamC), 1)]))
                
                if(split == "Directorate"){
                    
                    finalprint = paste0(vecprint$Improve, " (", vecprint$TeamN, ")")
                
                } else {
                
                    finalprint = vecprint$Improve
                }
                
                for (c in 1:length(finalprint)){
    ...
    if(length(miscprint[!is.na(miscprint) & !miscprint %in% c(recodelist$Omit, recodelist$Recode)]) > 0){
    
      myCat(c("<h3>Miscellaneous</h3>"))
     
      if(type == 1 & sum(miscprint %in% recodelist$Recode, na.rm = TRUE) > 0){ # only if it's improve one thing
    
        myCat(c("<p>We received ", sum(miscprint %in% recodelist$Recode, na.rm = TRUE),
                " comments which stated that nothing could be improved.</p>"))
      
      }
    
      miscprint = miscprint[!is.na(miscprint) & !miscprint %in% c(recodelist$Omit, recodelist$Recode)]
...
myCat('<script language="JavaScript" src="sorttable.js"></script>')
...
trustNPS = round((sum(Subset.P$Promoter == 5, na.rm=TRUE)/
...
trustNPSprev = round((sum(subset(mydatafirst, Time == Quarter-1)$Promoter == 5, na.rm=TRUE)/
...
trustNPSyear = round((sum(subset(mydatafirst, Time %in% (Quarter-4):(Quarter-1))$Promoter == 5, na.rm=TRUE)/
...
if(!is.na(trustNPS)){
    ...
    myCat(c("<p>The Friends and Family Test Score is calculated using the proportion of patients who would strongly
            recommend a service minus those who would not recommend, or who are indifferent. This quarter
            within the Trust it was ", trustNPS))
    
    if(!is.na(trustNPSprev)){
        
        if(!is.na(trustNPSyear)){
            
            myCat(c(". This compares with ", trustNPSprev, " in the previous quarter and ", trustNPSyear,
        ...
        if(!is.na(trustNPSyear)){
            ...
            myCat(c(". This compares with ", trustNPSyear,
                    " in the previous four quarters"))
    ...
    if(!999 %in% funcPlaceC & checkDiv){
        ...
        if(NPScheck){
            ...
            if(!is.na(NPS)){
                ...
                myCat(c("<p>The Friends and Family Test Score is calculated using the proportion of patients who would strongly
            recommend a service minus those who would not recommend, or who are indifferent. This quarter it was ", NPS))
        ...
        if(check1){
            ...
            if(NPScheckdir){
                ...
                if(!is.na(NPSdir)){
                    ...
                    myCat(c("<p><p>The Friends and Family Test Score is calculated using the proportion of patients who would strongly
            recommend a service minus those who would not recommend, or who are indifferent. This quarter it was ", NPSdir))
            ...
            mycomments(PlaceN="Directorate", PlaceC = d, type = 1, split = "Directorate") 
            
            myCat("<h2> Best thing individual comments </h2>")
            
            mycomments(PlaceN="Directorate", PlaceC = d, type = 2, split = "Directorate") 
            ...
            for (team in teamnames[teamnames!=1700]) {
                ...
                myCat(c("<li><a href=", team, ".html>", as.character(subset(mydatafirst, Time==Quarter & is.na(PO))$TeamN
                                                                     [tail(which(team==subset(mydatafirst, Time==Quarter & is.na(PO))$TeamC), 1)]),
                        "</a><br></li>"))
            ...
            for(team in teamcounter[!teamcounter %in% c(722, 1700)]) { 
                ...
                myCat(c("<title> ", as.character(subset(mydatafirst, Time==Quarter &
                                                            is.na(PO))$TeamN[tail(which(team==subset(mydatafirst,
                                                            Time==Quarter & is.na(PO))$TeamC), 1)]), " </title>"))
                ...
                myCat(c("<p>In ", as.character(subset(mydatafirst, Time==Quarter)$TeamN[tail(which(team==subset(mydatafirst,
                                                        Time==Quarter & is.na(PO))$TeamC), 1)]), " we received ",
                        length(subset(mydatafirst, TeamC==team&Time==Quarter)$Service), " responses. Of these ",
                        length(subset(mydatafirst, TeamC==team&Time==Quarter&SU==1)$Service), " were from carers.</p>"))
                ...
                mycomments(PlaceN="TeamC", PlaceC = team, type = 1, split = "Team") 
                
                myCat("<h2> Best thing individual comments </h2>")
                
                mycomments(PlaceN="TeamC", PlaceC = team, type = 2, split = "Team") 
...
#                 }
                ...
                for (team in teamnames) {
                    ...
                    myCat(c("<li><a href=", team, ".html>", as.character(subset(mydatafirst, Time==Quarter)$TeamN
                                                                         [tail(which(team==subset(mydatafirst, Time==Quarter &
                                                                                                      is.na(PO))$TeamC), 1)]), "</a><br></li>"))
...
bigFunction(name = "Local services", funcPlaceC = 0, funcPlaceN = "Local", URL = "local", barType = "main",
            dirVec = c(7, 2, 4, 6, 13, 14), funcLabels = mainlist)
